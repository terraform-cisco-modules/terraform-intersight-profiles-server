...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-profile-server-%s", uniqueId)
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} Server Profile.",
...
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/chassis/Profiles/%s", moid), expectedJSONTemplate, vars)
}
